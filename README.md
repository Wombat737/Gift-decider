...
**Citrus Lane** is the locked visual direction: lemon wash (`#FFF8E7` / `#FFFCF5`), teal brand (`#0D9488`) for primary and buy actions, amber only for chip-in and pledges (`#F59E0B`). Plus Jakarta Sans for UI; Fraunces for moment titles, empty states, and group-gift reveal. **Sunroom** (cream / moss / coral) is superseded. Success / purchased / reserved styling never appears on owner views.
...
Visual polish on the existing IA — owner vs giver pills, gift cards, empty states, invite copy. No information-architecture redesign. Group-gift reveal is now gated by a **reveal date**, not by funded. **Citrus Lane** tokens replace Sunroom (lemon wash, teal brand, amber pledges).
...
| **App identity** | `app.json` — name Gift Decider, slug `gift-decider`, scheme `giftdecider`, Citrus Lane teal splash / theme |
...
- **Citrus Lane** UI: Plus Jakarta Sans + Fraunces on web (Google Fonts); native uses system-ui / Georgia as the closest Expo stand-ins
